...
function convertSearchParamsToParamsObject(
  ...
  if (params.has('guestCount')) {
    ...
    listingsParams.guestCount = guestCountParam
      ? parseInt(guestCountParam, 10)
      : undefined;
  }

  if (params.has('roomCount')) {
    const roomCountParam = params.get('roomCount');
    listingsParams.roomCount = roomCountParam
      ? parseInt(roomCountParam, 10)
      : undefined;
  }

  if (params.has('bathroomCount')) {
    const bathroomCountParam = params.get('bathroomCount');
    listingsParams.bathroomCount = bathroomCountParam
      ? parseInt(bathroomCountParam, 10)
      : undefined;
  }
  if (params.has('locationValue')) {
    const locationValueParam = params.get('locationValue');
    listingsParams.locationValue = locationValueParam
      ? locationValueParam
      : undefined;
  }
