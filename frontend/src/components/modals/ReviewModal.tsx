...
const ReviewModal = () => {
  ...
  const onSubmit: SubmitHandler<FieldValues> = (RequestReview) => {
    setIsLoading(true);

    saveReview(listingId, RequestReview)
      .then(() => {
  ...
  const bodyContent = (
    ...
    >
      ...
      <div className="w-full h-48">
        <textarea
          className="w-full rounded outline-red-100 outline h-full resize-none"
          {...register('message')}
        ></textarea>
      </div>
    </div>
